...
Namespace Contensive.addons
    Public Class templateImporter
        ...
        '=================================================================================
        '
        Public Function GetContent(ByRef cp As CPBaseClass) As String
                ...
                If True Then
                    ...
                    Select Case FormID
                        Case FormDone
                            ...
                            '
                            result &= cp.Html.h1("Template Importer", , "tiTitle")
                            result &= cp.Html.p("The template was imported. Any issues will be listed here.", , "tiDescription")
                            result &= cp.Html.div("<a href=""" & cp.Site.GetProperty("adminURL") & "?af=4&cid=" & cp.Content.GetID("page templates") & "&id=" & TemplateID & """>Edit the new template</a>", , "tiFormCaption")
                            result &= cp.Html.div("<a href=""" & cp.Site.GetProperty("adminURL") & "?cid=" & cp.Content.GetID("shared Styles") & """>View Shared Styles</a>", , "tiFormCaption")
                        Case Else
                            '
                            ...
                            '
                            result &= cp.Html.h1("Template Importer", , "tiTitle")
                            result &= cp.Html.p("To create a new template, enter a template name and the URL where the page can be found. The template name must be unique in your website.", , "tiDescription")
                            '
                            result &= cp.Html.ul("<p>Select a template import Method.</p>" _
                                ...
                                & "")
                            '
                            result &= cp.Html.div("Enter the name of the new template.", , "tiFormCaption")
                            result &= cp.Html.div(cp.Html.InputText(RequestNameTemplateName, TemplateName), , "tiFormInput")
                            '
                            result &= cp.Html.div("Enter the URL where the new template can be found", , "tiFormCaption")
                            result &= cp.Html.div(cp.Html.InputText(RequestNameImportLink, DefaultLink), , "tiFormInput")
                            '
                            result &= cp.Html.div(buttonBar, , "tiButtonBar")
        ...
        Private Function ImportTemplate_Convert(ByVal cp As CPBaseClass, ByRef Doc As String, ByRef BasePath As String, ByRef SourceHost As String, ByRef TemplateID As Integer, ByRef ManageStyles As Boolean, ByRef BuildVersion As String, ByVal tempPath As String, ByVal importMethod As String, ByRef templateFilename As String) As String
            ...
            Try
                ...
                Do While ElementPointer < ElementCount
                        ...
                        Select Case UCase(TagName)
                            ...
                            Case "STYLE"
                                ...
                                Do While (Not TagDone) And (ElementPointer < ElementCount)
                                    ...
                                    If Not TagDone Then
                                        'UPGRADE_WARNING: Couldn't resolve default property of object Output.Add. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
                    ...
                    ElementPointer = ElementPointer + 1
                Loop
                'UPGRADE_WARNING: Couldn't resolve default property of object Output.Text. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
                        ...
                        Do While PtrStart <> 0 And LoopCnt < 100
                            ...
                            If PosURLStart <> 0 Then
                                ...
                                If PosURLEnd <> 0 Then
                                    ...
                                    'Call SaveSharedStyle(TemplateID, cp.Utils.DecodeUrl(RootRelativeLink), ImportStyles)
                                    '
                                    ' if link contains ?#, correct the destination filename
                                    '
                                    ...
                                    'RootRelativeLink = ConvertLinkToRootRelative(cp, Link, BasePath)
                                    'virtualfilename = convertRootRelativeLinkToPathFilename(cp, RootRelativeLink)
                                    'Call GetURLToFile(cp, SourceHost & RootRelativeLink, cp.Site.PhysicalWWWPath & virtualfilename, tempPath)
                                    'NameValue = Replace(NameValue, Link, RootRelativeLink, 1, -1, CompareMethod.Text)
        ...
        Private Function ConvertStyleLink_HandleImports(ByVal cp As CPBaseClass, ByRef Link As String, ByRef BasePath As String, ByRef SourceHost As String, ByRef BlockRootRelativeLinkList As String, ByRef TemplateID As Integer, ByRef tempPath As String) As String
            Try
                ...
                'UPGRADE_ISSUE: HTTPClass object was not upgraded. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6B85A2A7-FE9F-4FBE-AA0C-CF11AC86A305"'
